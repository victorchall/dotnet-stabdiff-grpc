...
generateCommand.SetHandler(async (steps, prompt, engineId, outdir, sampler, cnt, initImgPath, str) =>
{
    try
    {
        using (var initImage = await Image.LoadAsync(initImgPath))
        {
            ...
            var imgBytes = getImgBytes(initImage); // TODO async?
    ...
    {
        Console.WriteLine($"Unable to locate input image folder: {ex.Message}");
    }
    catch (FileNotFoundException ex)
    {
        Console.WriteLine($"Unable to locate input image file: {ex.Message}");
    }
},
